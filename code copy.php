...
Filename: code copy.php
Date Started: March 23, 2025
Date Finished: May 16, 2025
Purpose: this is the copy of code file for the JeepN! Tracking System. 
*/ -->
<?php
session_start();
include('dbcon.php');
...
if (isset($_POST['change_password_btn'])) {
    ...
    // 🔍 Get the role from Firebase to decide where to redirect
    $userData = $database->getReference("all_users/$uid")->getValue();
    $role = $userData['role'] ?? 'passengers'; // fallback to passengers
    ...
    if ($new_password === $retype_password) {
        ...
        if ($updatedUser) {
            ...
            header("Location: {$redirectPage}-list.php");
            exit();
        } else {
